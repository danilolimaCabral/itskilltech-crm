                      ...
                      <div key={s.id} style={{ background: s.read_at ? '#f8fafc' : '#fffbeb', border: `1px solid ${s.priority === 'high' ? '#fca5a5' : s.priority === 'low' ? '#bbf7d0' : '#e2e8f0'}`, borderRadius: 10, padding: '10px 14px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                          ...
                          <span style={{ fontSize: 11, color: '#94a3b8', marginLeft: 'auto' }}>
                            {(() => {
                              const d = new Date(parseInt(s.created_at));
                              return d instanceof Date && !isNaN(d.getTime()) ? d.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '—';
                            })()}
                          </span>
                          {s.read_at ? (
